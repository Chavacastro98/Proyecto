...
void taskFuenteFunc(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (localActiva && localPulsado) {
      ...
      /**
       * =================================================================================
       * MUESTREO ESTROBOSCÓPICO DE TIEMPO EQUIVALENTE (ETS - EQUIVALENT TIME SAMPLING)
       * =================================================================================
       * Reconstruye la forma de onda pulsada de corriente en 16 puntos (s_ondaETS)
       * tomando 1 muestra por pulso (o decimada a máx 10 Hz) con un offset temporal
       * progresivo delta_t_k = k * (tAlto - 4ms) / 16.
       * Esto elimina la necesidad de un ADC de mega-muestras por segundo y no sobrecarga
       * el bus I2C (máx 2.5% de ocupación de bus), brindando metrología de grado laboratorio.
       * =================================================================================
       */
      ...
      if (tocaMedirEstePulso && puedeMuestrearEnAlto) {
        // Cálculo del desplazamiento estroboscópico para el punto k (0..15)
        ...
        // Adquisición analógica de los dos shunts bajo cerrojo I2C
        if (takeI2CMutex(pdMS_TO_TICKS(10))) {
        ...
        if (lecturaOK) {
          ...
          float i1 = vs1 / 1.0f; // Shunt 1 = 1.0 Ohm
          float i2 = vs2 / 1.0f; // Shunt 2 = 1.0 Ohm
          ...
          if (takeDataMutex(pdMS_TO_TICKS(10))) {
            ...
            // Diagnóstico de Salud de la Celda Electroquímica
            float iTeorico = (localAmplitud / 4095.0f) * VCSS_IMAX_NOMINAL;
            if (iTeorico > 0.10f && iMuestra < (0.75f * iTeorico)) {
              estadoSaludCelda = CELDA_SATURADA; // Tensión de cumplimiento alcanzada / alta impedancia
            } else if (iMuestra > 0.20f && fabsf(i1 - i2) > (0.40f * iMuestra)) {
              estadoSaludCelda = CELDA_DESBALANCE; // Desbalance de corriente entre ramas
            } else {
        ...
        // Descontar tiempo consumido en el pulso alto (offset + ~3ms de lectura ADC)
        ...
        if (!puedeMuestrearEnAlto && tocaMedirEstePulso) {
          ...
          if (takeDataMutex(pdMS_TO_TICKS(10))) {
            float iPico = (localAmplitud / 4095.0f) * VCSS_IMAX_NOMINAL;
