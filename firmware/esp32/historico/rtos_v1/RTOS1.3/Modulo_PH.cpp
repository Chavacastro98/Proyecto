...
float leerVoltajePH(uint8_t canal) {
  ...
  if (takeI2CMutex(pdMS_TO_TICKS(50))) {
    ads.readADC_SingleEnded(canal); // Asentamiento de MUX para divisor de alta impedancia
...
void leerVoltajeCrudoDual(float &v0, float &v1) {
  ...
  if (takeI2CMutex(pdMS_TO_TICKS(100))) {
    // Lectura en bloque por canal para evitar conmutación alternada rápida de MUX
    ads.readADC_SingleEnded(0); // Asentamiento canal 0
    for (int i = 0; i < MUESTRAS; i++) {
      suma0 += ads.readADC_SingleEnded(0);
      delayMicroseconds(80);
    }
    ads.readADC_SingleEnded(1); // Asentamiento canal 1
    for (int i = 0; i < MUESTRAS; i++) {
...
bool ejecutarCalibracionPH(uint8_t tina, uint8_t punto, char *errorMsg,
  ...
  if (g_failsafe_latched || phInterlockActivo()) {
    Serial.println("[PH] ⚠️ Calibración bloqueada: Interlock activo (fuente "
  ...
  for (int i = 0; i < NUM_MUESTRAS; i++) {
    ...
    if (takeI2CMutex(pdMS_TO_TICKS(60))) {
      // 1 lectura de asentamiento (dummy) para conmutación de MUX
      ads.readADC_SingleEnded(canal);

      // Sobremuestreo de 5 conversiones para promediar la inyección de carga y EMI
      int32_t sumaRaw = 0;
      const int OVERSAMPLE_CAL = 5;
      for (int k = 0; k < OVERSAMPLE_CAL; k++) {
        sumaRaw += ads.readADC_SingleEnded(canal);
        delayMicroseconds(50);
      }
      giveI2CMutex();
