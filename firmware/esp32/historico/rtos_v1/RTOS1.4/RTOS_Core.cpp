...
void triggerFailSafe(uint8_t errCode, const char* reason) {
  ...
  // 1. Corte inmediato del relé VCSS (Aislamiento de potencia de +12V)
  digitalWrite(PIN_RELE_VCSS, !RELE_NIVEL_ACTIVO);
  estadoReleVDD = false;

  // 2. Apagado inmediato de TRIACs térmicos en Arduino Nano esclavo
  Serial2.println("0,0,0,0");

  // 3. Poner DAC MCP4725 a 0V con intento de toma rápida de mutex I2C (solo si fue inicializado)
