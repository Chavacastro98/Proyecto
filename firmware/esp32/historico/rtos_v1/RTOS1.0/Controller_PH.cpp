 ...
 * lecturas actuales, pendientes efectivas en porcentaje Nernst y banderas de calibración.
 */
...
/**
 * @brief Endpoint JSON que entrega el voltaje directo de la sonda para diagnóstico.
 */
...
/**
 * @brief Ejecuta el registro de un punto tampón (pH 4, 7 o 10) calculando las nuevas pendientes.
 */
static void handleDoCalPH() {
  ...
  if ((id != 1 && id != 2) || (punto != 4 && punto != 7 && punto != 10)) {
    server.send(400, "text/plain", "Parámetros de calibración inválidos");
    return;
  }

  bool ok = ejecutarCalibracionPH((uint8_t)id, (uint8_t)punto);

  float v = (id == 1) ? ((punto == 7) ? v7_1 : (punto == 4 ? v4_1 : v10_1))
                      : ((punto == 7) ? v7_2 : (punto == 4 ? v4_2 : v10_2));
  float m = (id == 1) ? m_ph1 : m_ph2;

  logSistema(LOG_LVL_INFO, "PH", "Calibracion Tina %d Punto pH %d: %s (V=%.3f, m=%.1f)",
             id, punto, ok ? "OK" : "FALLIDA", v, m);

  char json[96];
  snprintf(json, sizeof(json), "{\"ok\":%d,\"v\":\"%.3f\",\"slope\":\"%.3f\"}",
           ok ? 1 : 0, v, m);
  server.send(200, "application/json", json);
}
