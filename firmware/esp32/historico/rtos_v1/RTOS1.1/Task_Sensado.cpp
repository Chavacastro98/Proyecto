...
void taskSensadoFunc(void *pvParameters) {
  ...
  for (;;) {
    ...
    // 2. Sensado de corriente en shunts VCSS y compensación de ganancia cada 500 ms (25 ticks = 2 Hz)
    if (tickCount % 25 == 0) {
      if (localFuente && !localPH) {
        if (!localPulsado) {
          actualizarSensadoVCSS();
        }
        // NOTA METROLÓGICA: En modo pulsado, el sensado de corriente es delegado
        // exclusivamente a Task_Fuente para ejecutar muestreo estroboscópico en fase alta.
      }
    }
