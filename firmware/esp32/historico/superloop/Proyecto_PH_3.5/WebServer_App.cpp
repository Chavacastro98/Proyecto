...
/**
 * =================================================================================
 * IMPLEMENTACIÓN DEL SERVIDOR WEB — Versión 3.0 (WebServer_App.cpp)
 * =================================================================================
...
void inicializarWebServer() {
  // --- RESPUESTA SILENCIOSA FAVICON (evita retardos 404 en navegadores) ---
  ...
  server.on("/sensores", handleSensores); // v3.1: Diagnóstico de sensores

  // =====================================================================
  // ENDPOINT v3.1: Estado de todos los sensores/periféricos del sistema
  // Escanea I2C en caliente y reutiliza lecturas térmicas SPI existentes
  // Se llama BAJO DEMANDA (botón manual, sin auto-refresh)
  // =====================================================================
  server.on("/data_sensors", []() {
    char json[192];

    // --- Escaneo I2C en caliente (no destructivo) ---
    // Wire.beginTransmission + endTransmission retorna 0 si ACK recibido
    ...
    // Si la temperatura es <= 0 o >= 150, se considera falla del sensor
    int tc[4];
  ...
  // Datos de los 4 canales térmicos: temperatura actual, setpoint, potencia TRIAC y estado
  server.on("/data_t", []() {
    char json[256];
    int offset = 0;
    ...
    for (int i = 0; i < 4; i++) {
      ...
      offset += snprintf(json + offset, sizeof(json) - offset,
                         "{\"t\":%.1f,\"sp\":%.1f,\"p\":%d,\"run\":%d}%s", temp, sp,
                         pot, act ? 1 : 0, (i < 3) ? "," : "");
    }
  ...
  // Estado actual de la fuente de corriente (v3.5 con telemetría de shunts, lazo cerrado y relé)
  server.on("/data_f", []() {
    char json[288];
  ...
  // Activar / desactivar compensación en lazo cerrado VCSS (v3.5)
  server.on("/set_comp_f", []() {
    if (!server.hasArg("v")) {
      server.send(400, "text/plain", "Parámetro v faltante");
      return;
    }
  ...
  // Auto-calibrar transconductancia VCSS con carga de prueba de 1.50 A (v3.5)
  server.on("/cal_vcss", []() {
  ...
  server.on("/act_f", []() {
    ...
    // Conmutación con protocolo Zero-Current Switching y aislamiento de relé +12V
  ...
  // Parámetros: ?id=1 o 2 (tina) y ?p=7, 4 o 10 (buffer de referencia)
      ...
      if (punto == 7) {
        ...
        if (fabsf(v4_1 - v7_1) > 0.01f) {
          if (modo == 1) {
            ...
            // Calibración completa solo si también tenemos pH 10
...
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;background:#0b1120;color:#e2e8f0;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;}
...
/* Panel VCSS v3.5 */
.vcss-panel{background:rgba(15,23,42,0.6);border-radius:14px;padding:16px;margin-top:16px;border:1px solid rgba(56,189,248,0.15);text-align:left;}
.vcss-hdr{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;font-size:0.7em;color:#94a3b8;font-weight:700;letter-spacing:0.5px;}
...
.vcss-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px;}
.vcss-item{background:#0b1120;padding:10px 6px;border-radius:10px;text-align:center;border:1px solid rgba(51,65,85,0.4);}
.vcss-lbl{font-size:0.62em;color:#64748b;font-weight:700;margin-bottom:3px;text-transform:uppercase;}
.vcss-val{font-size:1.05em;font-weight:900;color:#38bdf8;font-variant-numeric:tabular-nums;}
.vcss-sub{font-size:0.65em;color:#94a3b8;font-weight:600;margin-top:3px;}
.vcss-actions{display:flex;gap:8px;}
.btn-vcss{flex:1;padding:9px 10px;background:#1e293b;border:1px solid #334155;color:#cbd5e1;border-radius:8px;font-size:0.72em;font-weight:800;cursor:pointer;transition:all 0.2s;}
...
<body>
    <div class='container'>
        <div class='hdr'>
            <a href='/' class='nav-back'>&larr; Men&uacute;</a>
            <span class='hdr-t'>Fuente de Corriente &middot; v3.5</span>
            <div></div>
        </div>
        <div class='card'>
            ...
            <!-- Panel Sensado VCSS Shunts (v3.5) -->
            <div class='vcss-panel'>
                <div class='vcss-hdr'>
                    <span>MONITOREO VCSS &middot; SHUNTS DUALES</span>
                    <div style='display:flex;gap:6px;'>
                        <span class='vcss-badge' id='rele-badge'>RELÉ +12V: AISLADO</span>
                        <span class='vcss-badge' id='comp-badge'>LAZO CERRADO: ACTIVO</span>
                    </div>
                </div>
                <div class='vcss-grid'>
                    <div class='vcss-item'>
                ...
                <div class='vcss-actions'>
                    <button class='btn-vcss' id='btn-comp' onclick='toggleComp()'>Desactivar Lazo</button>
                    <button class='btn-vcss btn-cal' onclick='calibrarVCSS()'>Auto-Calibrar Shunts</button>
                </div>
            </div>
        </div>
    </div>
    <script>
        ...
        function loadFuenteData(){
            ...
            fetch('/data_f').then(function(r){return r.json();}).then(function(d){
                ...
                // Datos de Telemetría Shunt VCSS
                document.getElementById('i1-val').innerText = parseFloat(d.i1).toFixed(2) + ' A';
                document.getElementById('i2-val').innerText = parseFloat(d.i2).toFixed(2) + ' A';
                document.getElementById('v1-val').innerText = parseFloat(d.vs1).toFixed(3) + ' V';
                document.getElementById('v2-val').innerText = parseFloat(d.vs2).toFixed(3) + ' V';
                document.getElementById('itot-val').innerText = parseFloat(d.i_real).toFixed(2) + ' A';
                document.getElementById('gm-val').innerText = 'Gm: ' + parseFloat(d.gm).toFixed(3) + ' S';
                ...
                if (compActivo) {
                    ...
                    compBadge.innerText = 'LAZO CERRADO: ACTIVO';
                    btnComp.innerText = '⚙ Desactivar Lazo';
                } else {
                    compBadge.className = 'vcss-badge off';
                    compBadge.innerText = 'LAZO ABIERTO (IDEAL)';
                    btnComp.innerText = '⚙ Activar Lazo';
                }

                var releBadge = document.getElementById('rele-badge');
                if (d.rele == 1) {
                    releBadge.className = 'vcss-badge';
                    releBadge.innerText = 'RELÉ +12V: ENERGIZADO';
                } else {
        ...
        function toggleComp(){
            var nv = compActivo ? 0 : 1;
            fetch('/set_comp_f?v=' + nv).then(function(){ loadFuenteData(); });
        }

        function calibrarVCSS(){
            if(!confirm('¿Iniciar auto-calibración de transconductancia VCSS?\nSe aplicará una corriente de prueba de 1.50 A durante 1 segundo.')) return;
            fetch('/cal_vcss').then(function(r){return r.json();}).then(function(d){
                if(d.ok==1){
                    alert('¡Calibración exitosa!\nFactor de Ganancia Gm: ' + d.gm + '\nGuardado en NVS.');
                } else {
                    alert('Falla en la calibración. Verifique la carga conectada y el circuito.');
                }
                loadFuenteData();
            }).catch(function(){alert('Error de comunicación.');});
        }

        setInterval(loadFuenteData, 1500); 
        loadFuenteData();
    </script>
</body></html>)rawliteral";
...
// =================================================================================
// DIAGNÓSTICO DE SENSORES v3.1 — INTERFAZ WEB
// =================================================================================
// Página de escaneo bajo demanda (botón manual) de los 8 periféricos del
// sistema. No usa auto-refresh para evitar tráfico I2C durante conexiones
// y desconexiones de hardware en caliente.
// =================================================================================
...
void handleSensores() { server.send_P(200, "text/html", HTML_SENSORES); }
