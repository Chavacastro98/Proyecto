...
void setEstadoFuente(bool encender) {
  ...
  if (encender) {
    ...
    // Limpieza estricta de integradores y temporizadores de lazos
    resetLazoDC();
    resetLazoPulsado();
  }
}
...
void conmutarModoFuente(bool usarPulsado) {
  if (takeDataMutex(pdMS_TO_TICKS(20))) {
    ...
    if (usarPulsado) {
      estadoPICorriente = PI_STATE_OFF;
      resetLazoDC();      // Limpiar integrador del lazo DC al salir de continua
      resetLazoPulsado();
    } else {
      resetLazoPulsado(); // Limpiar lazo pulsado al salir de pulsado
      if (fuenteActiva) {
        resetLazoDC();
      }
    }
