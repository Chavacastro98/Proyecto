...
/**
 * =================================================================================
 * CONFIGURACIÓN GLOBAL DEL SISTEMA (config.h) — Versión RTOS 2.0
 * =================================================================================
...
// =================================================================================
// PARÁMETROS DE CONTROL DE CORRIENTE Y MUESTREO ETS (RTOS 2.0)
// =================================================================================
#define VCSS_PI_BLANKING_MS   0UL    // 0 ms: respuesta analógica inmediata
#define VCSS_SOFT_START_MS    0UL    // 0 ms: arranque por escalón directo (nucleación óptima y filtrado por Cdl)
#define VCSS_PI_KP            0.12f  // Ganancia Proporcional
#define VCSS_PI_KI            0.04f  // Ganancia Integral
#define ETS_NUM_PUNTOS        16     // Puntos de reconstrucción de onda en tiempo equivalente

enum EstadoPI_t : uint8_t {
  PI_STATE_OFF       = 0, // Fuente apagada o modo pulsado
  PI_STATE_RAMP      = 1, // Rampa (bypass)
  PI_STATE_BLANKING  = 2, // Asentamiento
  PI_STATE_LOCKED    = 3  // Lazo analógico puro calibrado activo
};

enum SaludCelda_t : uint8_t {
  CELDA_OK           = 0, // Celda galvánica operando según modelo teórico
  CELDA_SATURADA     = 1, // Voltaje de cumplimiento alcanzado (pasivación o alta resistencia)
  CELDA_DESBALANCE   = 2  // Desbalance térmico/corriente significativo entre shunts
};
 ...
 *   1. ADS1115 (Dirección 0x48): Sensor dedicado de pH en Canal A1 (Canal A0 eliminado) y Shunts de corriente (A2/A3).
 ...
 * - Rango de entrada DAC: 0.00 V a 3.53 V -> Rango nominal de salida: 0.00 A a 7.06 A (limitado por firmware a 3.50 A).
 */
#define VCSS_GM_NOMINAL     2.00f   // Transconductancia total nominal (2.00 S)
#define VCSS_VREF_DAC       3.53f   // Tensión de fondo de escala del DAC MCP4725 ajustada a riel físico (3.53 V)
#define VCSS_IMAX_NOMINAL   (VCSS_VREF_DAC * VCSS_GM_NOMINAL) // 7.06 A teóricos a 4095 LSB
...
// =================================================================================
#define VREF                  3.53f   // Tensión real del riel de alimentación calibrado (3.53 V)
#define PH_OFFSET_TEORICO     1.765f  // Offset teórico pH 7 neutro a VDD/2 (3.53 V / 2 = 1.765 V)
#define PH_PENDIENTE_TEORICA  4.242f
#define PH_CAL_MAX_DISPERSION_MV  80.0f   // Umbral máximo de dispersión pico a pico admisible (mV)
...
// --- Variables Módulo pH (RTOS 2.0 - Modo Pseudo-Diferencial Kelvin Ground A1-A0) ---
#define ADS_PH_MODO_DIFERENCIAL 1       /**< 1: Modo Pseudo-Diferencial A1-A0 (Po en A1, GND2 en A0 con divisores 100k/200k); 0: Single-Ended A1 */
#define ADS_CH_PH             1       /**< Canal A1 del ADS1115 dedicado a la señal Po de pH */
#define ADS_CH_PH_REF         0       /**< Canal A0 del ADS1115 dedicado a la referencia de masa limpia GND2 del PH-4502C */
extern volatile bool          phModuloActivo;
extern volatile uint8_t       tipoCalPH;
extern volatile float         v7_ph, v4_ph, v10_ph, m_ph, mAcida_ph, mBasica_ph, phActual;
extern volatile bool          calibradoPH;
extern volatile float         voltajeADC_ph;      /**< Voltaje analógico en entrada ADS1115 Canal A1 (V) */
extern volatile float         voltajeSonda_ph;    /**< Voltaje real acondicionado de sonda (V) */
extern volatile float         voltajeCrudoPH;     /**< Voltaje directo para ajuste de offset con BNC en corto */

// Alias de compatibilidad para simplificar la transición
#define phActual1             phActual
#define phActual2             phActual
#define voltajeADC1           voltajeADC_ph
#define voltajeADC2           voltajeADC_ph
#define voltajeSonda1         voltajeSonda_ph
#define voltajeSonda2         voltajeSonda_ph
#define tipoCalPH1            tipoCalPH
#define tipoCalPH2            tipoCalPH
#define calibradoPH1          calibradoPH
#define calibradoPH2          calibradoPH
#define v7_1                  v7_ph
#define v7_2                  v7_ph
#define v4_1                  v4_ph
#define v4_2                  v4_ph
...
#define m_ph1                 m_ph
#define m_ph2                 m_ph
#define mAcida1               mAcida_ph
#define mAcida2               mAcida_ph
#define mBasica1              mBasica_ph
#define mBasica2              mBasica_ph
...
extern volatile uint8_t estadoPICorriente;       /**< EstadoPI_t: OFF, RAMP, BLANKING, LOCKED */
extern volatile uint8_t estadoSaludCelda;        /**< SaludCelda_t: OK, SATURADA, DESBALANCE */
extern volatile float   s_ondaETS[ETS_NUM_PUNTOS];/**< Onda estroboscópica de corriente reconstruida */
extern volatile float   energiaTermicaTotal_Wh;  /**< Energía acumulada en calentadores térmicos (Wh) */
