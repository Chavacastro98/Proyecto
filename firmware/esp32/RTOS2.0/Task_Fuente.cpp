/**
 * =================================================================================
 * TAREA FREERTOS: SALIDA DE CORRIENTE VCSS (Task_Fuente.cpp) — RTOS 2.0
 * =================================================================================
...
void taskFuenteFunc(void *pvParameters) {
  Serial.println("[TASK_FUENTE] Tarea iniciada en Core 1 (Prioridad 4) — RTOS 2.0.");

  uint32_t lastHB = millis();
  static bool s_prevActiva = false;
  static bool s_prevPulsado = false;
  ...
  for (;;) {
    ...
    // Se preserva el estado del ciclo anterior en caso de contención temporal de mutex
    // para evitar caídas espurias de corriente durante transacciones HTTP en Core 0.
    bool localActiva = s_prevActiva;
    bool localPulsado = s_prevPulsado;

    if (takeDataMutex(pdMS_TO_TICKS(20))) {
      localActiva = fuenteActiva;
      localPulsado = modoPulsado;
      s_prevActiva = localActiva;
      s_prevPulsado = localPulsado;
      giveDataMutex();
    }
