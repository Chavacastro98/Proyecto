/**
 * =================================================================================
 * CONTROLADOR: SALIDA DE CORRIENTE VCSS (Controller_Fuente.cpp) — RTOS 2.0
 * =================================================================================
...
static void handleSetF() {
  ...
  bool persistirNVS = (!server.hasArg("s") || server.arg("s") != "0");
  if (nvsKey != NULL && persistirNVS) {
...
static void handleCalVCSS() {
  ...
  char json[160];
  snprintf(json, sizeof(json), "{\"ok\":%d,\"gm\":%.4f%s}", ok ? 1 : 0, factorGananciaVCSS, ok ? "" : ",\"err\":\"Corriente fuera de rango (celda abierta o desconectada)\"");
  server.send(200, "application/json", json);
}
