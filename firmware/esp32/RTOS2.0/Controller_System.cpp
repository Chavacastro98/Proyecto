...
static void handleDataAll() {
  ...
  // 3. Snapshot pH Dedicado (RTOS 2.0 - Canal A1)
  float ph_val = 7.0f;
  float v_ph = 1.765f, vs_ph = 2.50f;
  bool ph_on = false, cal_ph = false;
  uint8_t m_ph_mode = 0;
  float slope_m = PH_PENDIENTE_TEORICA, sAc = PH_PENDIENTE_TEORICA, sBa = PH_PENDIENTE_TEORICA;
  float nvs_v7 = PH_OFFSET_TEORICO, nvs_v4 = 2.0f, nvs_v10 = 1.3f;
  ...
  float sl = 100.0f;
  if (m_ph_mode == 1 && cal_ph) sl = fabsf(slope_m) / PH_PENDIENTE_TEORICA * 100.0f;
  else if (m_ph_mode == 2 && cal_ph) sl = (fabsf(sAc) + fabsf(sBa)) / (2.0f * PH_PENDIENTE_TEORICA) * 100.0f;
  ...
  snprintf(json, sizeof(json),
    ...
    "\"ph\":{\"p\":\"%.2f\",\"v\":\"%.3f\",\"vs\":\"%.2f\",\"on\":%d,\"m\":%d,"
             "\"sl\":\"%.1f\",\"c\":%d,\"il\":%d,"
             "\"v7\":\"%.3f\",\"v4\":\"%.3f\",\"v10\":\"%.3f\","
             "\"p1\":\"%.2f\",\"p2\":\"%.2f\",\"v1\":\"%.3f\",\"v2\":\"%.3f\","
             "\"m1\":%d,\"m2\":%d,"
             "\"sl1\":\"%.1f\",\"sl2\":\"%.1f\",\"c1\":%d,\"c2\":%d},"
    "\"env\":{\"t\":\"%.1f\",\"h\":\"%.0f\",\"p\":\"%.1f\"},"
    "\"fs\":{\"latched\":%d,\"code\":%d,\"reason\":\"%s\"}}",
    ...
    ph_val, v_ph, vs_ph, ph_on ? 1 : 0, (int)m_ph_mode, sl, cal_ph ? 1 : 0, il ? 1 : 0,
    nvs_v7, nvs_v4, nvs_v10,
    ph_val, ph_val, v_ph, v_ph, (int)m_ph_mode, (int)m_ph_mode, sl, sl, cal_ph ? 1 : 0, cal_ph ? 1 : 0,
    env_t, env_h, env_p,
