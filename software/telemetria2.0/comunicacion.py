...
class ComunicacionMixin:
        ...
        def _ping():
            try:
                ...
                if r.status_code == 200:
                    self.conectado = True
                    if hasattr(self, 'destellar_rx'):
                        self.root.after(0, self.destellar_rx)
                    try:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if hasattr(self, 'show_toast'):
                    self.show_toast(f"Conectado a ESP32 ({ip})", tipo="success")
                else:
    ...
    def _generar_datos_simulados(self, t_rel):
        ...
        # pH Simulado - RTOS 2.0 Canal Dedicado A1
        v_adc_sim = round(1.765 + (7.00 - ph1_val) * 0.05916, 4)
        v_sonda_sim = round(v_adc_sim + 0.735, 4)
        d_ph = {
            "p": ph1_val,
            "p1": ph1_val,
            "p2": ph1_val,
    ...
    def _bucle_adquisicion(self):
        ...
        while self.grabando:
            ...
            if d_t and d_ph and d_f and d_env:
                ...
                # Fila de Telemetría Completa RTOS 1.3 / 2.0
                try:
                    fila_tel = [
                        t_iso, t_rel,
                        ...
                        # T1 y TRIAC 1
                        d_t[0].get("t", 0.0), d_t[0].get("sp", 0.0), d_t[0].get("run", 0),
                        p1_pct, a1_deg, d1_us, w1,
                        # T2 y TRIAC 2
                        d_t[1].get("t", 0.0), d_t[1].get("sp", 0.0), d_t[1].get("run", 0),
                        p2_pct, a2_deg, d2_us, w2,
                        # T3 y TRIAC 3
                        d_t[2].get("t", 0.0), d_t[2].get("sp", 0.0), d_t[2].get("run", 0),
                        p3_pct, a3_deg, d3_us, w3,
                        # T4 y TRIAC 4
                        d_t[3].get("t", 0.0), d_t[3].get("sp", 0.0), d_t[3].get("run", 0),
                        p4_pct, a4_deg, d4_us, w4,
                        # pH
                        d_ph.get("p1", d_ph.get("p", 7.0)),
                        d_ph.get("p2", d_ph.get("p", 7.0)),
                        d_ph.get("on", 1),
                        d_ph.get("m1", d_ph.get("m", 1)),
                        d_ph.get("m2", d_ph.get("m", 1)),
                        d_ph.get("sl1", d_ph.get("sl", 100.0)),
                        d_ph.get("sl2", d_ph.get("sl", 100.0)),
                        d_ph.get("il", 0),
                        # Fuente
                        d_f.get("act", 0), d_f.get("modo", 0),
                    ...
                    print(f"[WARN] Error procesando telemetría CSV: {ex_tel}")
    ...
    def _detectar_incidencias_sensores(self, t_rel, d_t, d_ph, d_f, d_env, nom_etapa):
        ...
        if d_ph and int(d_ph.get("on", 0)) == 1:
            for idx_ph, key_ph in enumerate(["p1", "p2"]):
                try:
                    ph_val = float(d_ph.get(key_ph, d_ph.get("p", 7.0)))
                except (ValueError, TypeError):
                    continue
