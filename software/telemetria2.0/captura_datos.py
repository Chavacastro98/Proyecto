...
def main():
    ...
    with open(filename, mode="w", newline="", encoding="utf-8") as csv_file:
        ...
        print("\n🚀 INICIANDO CAPTURA DE DATOS EN TIEMPO REAL (Telemetría 2.0 - RTOS 2.0)")
        print("💡 Presiona Ctrl+C en cualquier momento para detener y finalizar el archivo.\n")
        print(
            f"{'Tiempo':>8} | {'T1 Limp':>8} | {'T2 Decap':>8} | {'T3 Hull':>8} | {'T4 Níquel':>9} | {'pH (A1)':>8} | {'I Real (A)':>10} | {'Ambiente':>10}"
        )
        print("-" * 88)
        ...
        try:
            while True:
                ...
                ph_val = ph_data.get("p", ph_data.get("p1", "--"))
                ph_txt = str(ph_val) if ph_data.get("on") == 1 else "OFF"
                i_txt = f"{i_real:.2f} A" if f_data.get("act") == 1 else "0.00 A"
                env_txt = f"{env_data.get('t')}°C/{env_data.get('h')}%"

                print(
                    f"{t_relativo:>7.1f}s | {t1_txt:>8} | {t2_txt:>8} | {t3_txt:>8} | {t4_txt:>9} | {ph_txt:>8} | {i_txt:>10} | {env_txt:>10}"
                )
